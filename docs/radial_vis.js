function processData(csvFile, containerId, title) {
    ...
    // d3.csv(csvFile).then(data => {
        ...
        const annotations = [
            { label: "2 Points", angle: -0.5 * Math.PI, radius: shotRadius("twoPointShots") },
            { label: "3 Points", angle: -0.5 * Math.PI, radius: shotRadius("threePointShots") }
        ];
        ...
        annotations.forEach(d => {
            svg.append("text")
                ...
                .attr("text-anchor", "middle")
                .attr("fill", "black")
                //.attr("transform", `translate(-100, 0))` ) // rotate(90, ${Math.cos(d.angle) * d.radius}, ${Math.sin(d.angle) * d.radius})`)
                .text(d.label);
        });
...
//def a function to clear the chart
